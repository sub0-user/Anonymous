...
class InviteCodecTest {
    ...
    fun `private invite roundtrip without expiry`() {
        val invite = PrivateRoomInvite(roomId, service, founder, wrappedKey, founderPublicKey)
    ...
    fun `tampered invites are rejected`() {
        val privateText =
            InviteCodec.encode(PrivateRoomInvite(roomId, service, founder, wrappedKey, founderPublicKey))
        val publicText =
    ...
    fun `invalid addresses are rejected`() {
        val bad = PrivateRoomInvite(roomId, "not-an-onion", founder, wrappedKey, founderPublicKey)
        assertThrows(IllegalStateException::class.java) { InviteCodec.encode(bad) }
    }

    @Test
    fun `bad base64 and bad entry keys are rejected`() {
        val badKey = PrivateRoomInvite(roomId, service, founder, ByteArray(0), founderPublicKey)
