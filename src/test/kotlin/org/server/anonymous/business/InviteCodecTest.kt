...
class InviteCodecTest {
    ...
    fun `private invite roundtrip without expiry`() {
        val invite = PrivateRoomInvite(roomId, service, founder, authPrivate, wrappedKey)
    ...
    fun `private invite roundtrip with expiry`() {
        val invite = PrivateRoomInvite(roomId, service, founder, authPrivate, wrappedKey, 1_752_000_000L)
    ...
    fun `public invite roundtrip`() {
        ...
        val invite = PublicRoomInvite(roomId, service, founder, entryKey)
        val decoded = InviteCodec.decode(InviteCodec.encode(invite))
    ...
    fun `tampered invites are rejected`() {
        val privateText = InviteCodec.encode(PrivateRoomInvite(roomId, service, founder, authPrivate, wrappedKey))
        val publicText = InviteCodec.encode(PublicRoomInvite(roomId, service, founder, EntryKey.generate()))
    ...
    fun `invalid addresses are rejected`() {
        val bad = PrivateRoomInvite(roomId, "not-an-onion", founder, authPrivate, wrappedKey)
        assertThrows(IllegalStateException::class.java) { InviteCodec.encode(bad) }
    }

    @Test
    fun `bad base64 and bad entry keys are rejected`() {
        val badKey = PrivateRoomInvite(roomId, service, founder, ByteArray(31), wrappedKey)
        assertThrows(IllegalStateException::class.java) { InviteCodec.decode(InviteCodec.encode(badKey)) }
        val badEntry = PublicRoomInvite(roomId, service, founder, "NOTAKEY")
